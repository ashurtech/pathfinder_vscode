...
import { SchemaEnvironment, SchemaEnvironmentGroup } from '../src/types';
...
describe('ConfigurationManager - Credential Webview Functionality', () => {
    ...
    describe('checkMissingCredentials', () => {
        ...
        it('should ignore environments without auth configuration', async () => {            const environment: SchemaEnvironment = {
                id: 'env-1',
                name: 'Test Environment',
                schemaId: 'schema-1',
                baseUrl: 'https://api.test.com',
    ...
    describe('groupItemsBySchema', () => {
        it('should group environments and groups by schema name', async () => {
            ...
            const groupedItems = await (configManager as any).groupItemsBySchema([environment], [group]);
    ...
    describe('getSchemaNameForEnvironment', () => {
        it('should return schema name for existing schema', async () => {            const environment: SchemaEnvironment = {
                id: 'env-1',
                name: 'Test Environment',
                schemaId: 'schema-1',
                baseUrl: 'https://api.test.com',
            ...
            // Use reflection to access private method
            const schemaName = await (configManager as any).getSchemaNameForEnvironment(environment);

            expect(schemaName).toBe('Test API');
        });

        it('should return "Unknown Schema" for non-existing schema', async () => {            const environment: SchemaEnvironment = {
                id: 'env-1',
                name: 'Test Environment',
    ...
    describe('getSchemaNameForGroup', () => {
        it('should return schema name for existing schema', async () => {
            ...
            // Use reflection to access private method
            const schemaName = await (configManager as any).getSchemaNameForGroup(group);
    ...
    describe('importConfiguration integration', () => {
        it('should trigger credential webview for imported items with missing credentials', async () => {
            ...
            expect(showCredentialsWebviewSpy).toHaveBeenCalledWith(
